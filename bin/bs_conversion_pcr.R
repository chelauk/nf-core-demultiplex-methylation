...
# Read in the tables
CHH    = read.table(list.files( pattern = "^CHH_OB_"), 
                    sep = "\t", skip = 1, stringsAsFactors = FALSE)
CHG    = read.table(list.files( pattern = "^CHG_OB"), 
                    sep = "\t", skip = 1, stringsAsFactors = FALSE)
CpG_OB = read.table(list.files( pattern = "^CpG_OB"),
                    sep = "\t", skip = 1, stringsAsFactors = FALSE)
...
pdf(paste0(sample,"conversion.pdf"))
bc
dev.off()
